...
export class RepositoryCloningTrigger implements ApplicationTrigger<void, Application> {
  /**
  *
  */
    ...
    if (repository) {
      switch (repository.model.state) {
        ...
        default:
          listener.reject(
            new Error(
              'Unable to await termination of the cloning operation of ' +
              repository.toString() + ' because the requested ' +
              'repository is not awaiting to be cloned nor being cloned.'
            )
          )
          return
      }
